...
def ensure_wgs84(gdf):
    if gdf.crs is None:
        raise ValueError(
            "The shapefile has no CRS defined. Please add a .prj file or set a CRS before using this app."
        )
...
def get_edited_geometry(map_output):
    """
    Pull the most recent edited/drawn geometry from streamlit-folium.
    """
    if not map_output:
        return None
...
# =========================
# Custom editable draw control
# =========================
class EditableSelectedFeatureControl(MacroElement):
    """
    Injects a Leaflet.Draw toolbar that:
    - loads the selected feature into an editable FeatureGroup
    - allows true vertex editing on that selected feature
    - allows optional replacement by drawing a new geometry
    """

    def __init__(self, selected_feature_geojson):
        super().__init__()
        self._name = "EditableSelectedFeatureControl"
        self.selected_feature_geojson = json.dumps(selected_feature_geojson)

        self._template = Template(
            """
            {% macro script(this, kwargs) %}
            var map = {{ this._parent.get_name() }};
            var selectedFeature = {{ this.selected_feature_geojson }};

            // Use drawnItems because streamlit-folium / Leaflet.Draw integrations
            // commonly inspect this editable group.
            var drawnItems = new L.FeatureGroup();
            map.addLayer(drawnItems);

            // Add selected feature into editable group
            ...
            // If user draws a brand-new geometry, replace the selected feature with it
            map.on(L.Draw.Event.CREATED, function(e) {
                drawnItems.clearLayers();
                drawnItems.addLayer(e.layer);
            });

            // Keep edited geometry in drawnItems
            map.on(L.Draw.Event.EDITED, function(e) {
                // no-op; the geometry stays in drawnItems
            });

            // If deleted, editable group becomes empty
...
def make_map(gdf_wgs84, selected_idx=None, id_field=None):
    ...
    # Reference layer: all features
    GeoJson(
        data=json.loads(gdf_wgs84.to_json()),
        name="All features",
        tooltip=GeoJsonTooltip(
            fields=[f for f in tooltip_fields if f in gdf_wgs84.columns],
            aliases=[f for f in tooltip_fields if f in gdf_wgs84.columns],
        ),
    ...
    # Editable selected feature
    if selected_idx is not None:
        selected = gdf_wgs84.loc[[selected_idx]]
        selected_geojson = json.loads(selected.to_json())
        EditableSelectedFeatureControl(selected_geojson).add_to(m)
...
st.caption(
    "Upload a zipped shapefile, select a feature, vertex-edit or redraw its geometry on the map, and download the updated shapefile."
)
...
3. The selected feature is loaded into an **editable layer** on the map.
4. Use the edit toolbar to **drag vertices, add/remove vertices, or move the geometry**.
5. Optionally, draw a brand-new geometry to replace the selected one entirely.
6. Click **Apply edited geometry to selected feature**.
7. Download the updated shapefile as a ZIP.

**Notes**
- This version supports **true vertex editing** for the currently selected feature.
- It works best for **Point / LineString / Polygon** features.
- If your source data is not in WGS84, the app reprojects for map display and transforms edits back to the original CRS before saving.
...
if uploaded is not None:
    try:
        ...
        if gdf.crs is None:
            st.error(
                "This shapefile does not have a CRS. Please include a valid .prj file or assign a CRS before uploading."
            )
            st.stop()
    ...
    with left:
        ...
        st.write(
            f"**Geometry type(s):** {', '.join(sorted(set(st.session_state.gdf_current.geometry.geom_type.astype(str))))}"
        )

        possible_id_fields = [c for c in st.session_state.gdf_current.columns if c != "geometry"]
        default_id_idx = (
            possible_id_fields.index(st.session_state.id_field)
            if st.session_state.id_field in possible_id_fields
            else 0
        )
        chosen_id = st.selectbox(
            "Label field for feature selection",
            possible_id_fields,
        ...
        if (
            st.session_state.selected_index is None
            or st.session_state.selected_index not in st.session_state.gdf_current.index
        ):
            st.session_state.selected_index = 0

        current_label = (
            labels.iloc[st.session_state.selected_index]
            if st.session_state.selected_index in labels.index
            else labels.iloc[0]
        )
        ...
        chosen_label = st.selectbox(
            ...
            labels.tolist(),
            index=labels.tolist().index(current_label),
        )
        ...
        with st.expander("Preview attribute table"):
            st.dataframe(
                dataframe_for_display(st.session_state.gdf_current),
    ...
    with right:
        ...
        st.info(
            "The selected feature is loaded into the editable layer. Use the edit toolbar to drag vertices, add/remove vertices, move points, or draw a replacement geometry."
        )
        ...
        edited_geom_wgs84 = get_edited_geometry(map_output)

        if edited_geom_wgs84 is not None:
            st.success(f"Detected edited geometry: {edited_geom_wgs84.geom_type}")
            st.code(
                edited_geom_wgs84.wkt[:1200]
                + ("..." if len(edited_geom_wgs84.wkt) > 1200 else ""),
                language="text",
            )
        else:
            st.warning(
                "No edited geometry detected yet. Use the edit handles to change vertices, or draw a new geometry to replace the selected feature."
            )

        col_apply, col_download = st.columns([1, 1])

        with col_apply:
            if st.button("Apply edited geometry to selected feature", type="primary", use_container_width=True):
                if edited_geom_wgs84 is None:
                    st.error("Please edit the selected geometry or draw a replacement on the map first.")
                else:
                    try:
                        edited_series = gpd.GeoSeries(
                            [edited_geom_wgs84], crs="EPSG:4326"
                        ).to_crs(st.session_state.source_crs)
                        new_geom = edited_series.iloc[0]
        ...
        with col_download:
            try:
                zip_bytes = save_gdf_as_shapefile_zip(
                    st.session_state.gdf_current,
                    output_name="edited_shapefile",
                )
                st.download_button(
